...
function rmCommand(path: string): string {
  return `rm -rf ${shQuote(path)}`;
}

const PRUNE_TITLES: Record<PruneConfidence, string> = {
  safe: "SAFE TO DELETE — backups, nested copies, same-catalog duplicates",
  optional: "OPTIONAL — identical copies on agents you may not use",
  review: "REVIEW — oversized or broken; trim or fix, don't blindly delete",
};
...
export function formatPrune(plan: PrunePlan, options: { max?: number } = {}): string {
  ...
  const lines = [
    ...
    "",
    pc.dim("skillint never deletes files. Copy a command below, or pass --script."),
    "",
  ...
  for (const [confidence, items] of sections) {
    ...
    for (const item of items.slice(0, max)) {
      ...
      if (confidence === "review") {
        lines.push(`      ${pc.dim(`# ${rmCommand(item.deletePath)}`)}`);
      } else {
        lines.push(`      ${rmCommand(item.deletePath)}`);
      }
    }
    if (items.length > max) {
  ...
  if (plan.drop.length === 0) {
    ...
    lines.push("", pc.dim("Safe commands only: `skillint prune --script`"));
  }
...
export function formatPruneScript(plan: PrunePlan): string {
  ...
  const lines = [
    ...
    "# Safe deletions only: backups, nested copies, same-catalog duplicates.",
    "set -eu",
  ];
  if (paths.length === 0) {
    lines.push("echo 'Nothing safe to delete.'");
    return `${lines.join("\n")}\n`;
  }
  for (const path of paths) {
    const item = byPath.get(path);
    if (item) lines.push(`# ${item.file.name}: ${item.reason}`);
    lines.push(rmCommand(path));
  }
  return `${lines.join("\n")}\n`;
}
